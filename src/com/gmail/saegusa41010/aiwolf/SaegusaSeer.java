...
public class SaegusaSeer extends SaegusaBase {
	int comingoutDay;
	boolean isCameout;
	...
	List<Agent> grayList;
	List<Agent> semiWolves = new ArrayList<>();//いらない説
	...
	public String talk() {
		...
		if (!isCameout && (day >= comingoutDay
				|| (!divinationQueue.isEmpty() && divinationQueue.peekLast().getResult() == Species.WEREWOLF)
				|| isCo(Role.SEER))) {
	...
	public Agent divine() {
		//人狼候補は確定できるものは占わない->grayを占う
		if(!grayList.isEmpty()){
			return randomSelect(grayList);
		}
		// 人狼候補がいればそれらからランダムに占う->自分の中で確信してるやつは占わなくて良い
	...
	protected void chooseVoteCandidate() {
			...
			if (!aliveWolves.contains(voteCandidate)) {
				...
				if (canTalk) {
					talkQueue.offer(new Content(
							new RequestContentBuilder(null, new Content(new VoteContentBuilder(voteCandidate)))));
				}
			}
			return;
		}
		...
		for (Agent a : aliveOthers) {
			if (comingoutMap.get(a) == Role.SEER) {
				werewolves.add(a);
				blackList.add(a);//自分が占い師なのに占いって言ってるやつは人狼と確定
				if(grayList.contains(a)) {
					grayList.removeIf(ag -> ag==a);//grayリストから除外
				}
			}
		}
		// 偽霊媒師
		for (Judge j : identList) {
			...
			if ((myDivinationMap.containsKey(j.getTarget()) && j.getResult() != myDivinationMap.get(j.getTarget()))) {
				if (isAlive(agent) && !werewolves.contains(agent)) {
					...
					blackList.add(agent);//嘘つき霊媒師も黒にする
					if(grayList.contains(agent)) {
						grayList.removeIf(ag -> ag==agent);//grayリストから除外
					}
				}
			}
		}
			...
			if (whiteList.contains(a)) {
				if (!possessedList.contains(a)) {
					...
					if(blackList.contains(a)) {
						blackList.removeIf(ag -> ag==a);//blackリストから除外
					}
				}
			} else {
				semiWolves.add(a);
